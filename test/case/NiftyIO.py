...
def readNifty(filePath,CoordinateOrder='xyz'):
 ...
 # vol,_=readNifty(filePath)
    """
    image = sitk.ReadImage(filePath)#该方法专门用于读取医学图像
    # print("image:",image)
    ...
    # print("or:",image.GetOrigin())
    # print("sp:",image.GetSpacing())
    # print("di:",image.GetDirection())
    ...
    # 由于调用sitk.GetArrayFromImage()方法，坐标发生变化，因此需调用np.transpose()方法来进行转化。
    volume_zyx = sitk.GetArrayFromImage(image)
    if CoordinateOrder=='xyz':
        #np.transpose()函数，对原来数组进行转置，第一个元素变成最后一个元素依次类推
